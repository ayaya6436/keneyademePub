...
/* ==========================================================
  Gmap
=========================================================== */
if ( !function_exists('redl_gmap_function')) {
  function redl_gmap_function( $atts, $content = NULL ) {
    ...
    if ($gmap_common_marker) {
      ...
      $common_marker = VCTS_PLUGIN_ASTS . '/images/map-marker.png';
    }
    // Zoom
    ...
    // Google Map Height
    if ( $gmap_height ) {
      $inline_style .= '.redl-gmap-'. $e_uniqid .' {';
      $inline_style .= ( $gmap_height ) ? 'height:'. redel_check_px($gmap_height) .';' : '';
      $inline_style .= '}';
    }

    // add inline style
    add_inline_style( $inline_style );
    $styled_class  = ' redl-gmap-'. $e_uniqid;

    wp_enqueue_script( 'redel-googlemap', VCTS_PLUGIN_ASTS . '/jquery.googlemap.js', array( 'jquery' ), '1.5.0', true );
    wp_enqueue_script( 'redel-map-api', '//maps.googleapis.com/maps/api/js'. $gmap_api .'', array( 'jquery' ), '', true );

    // Turn output buffer on
    ob_start();

    echo '<div id="map-'. $e_uniqid .'" class="redl-google-map '. $custom_css . $class . $styled_class .'"></div>';
?>
...
add_shortcode( 'redl_gmap', 'redl_gmap_function' );
